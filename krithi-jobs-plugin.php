  ...
  if(!class_exists('CutomJobsPlugin')){
    ...
    class CutomJobsPlugin{
      ...
      public function __construct() {
        ...
        add_action('wp_ajax_my_delete_post', array($this, 'delete_job'));
        ...
        register_activation_hook( __FILE__, array($this,'job_table'));
      }

      function scripts(){
        wp_enqueue_style('apply-form', plugin_dir_url( __FILE__ ) . 'style.css' );
        wp_enqueue_script('apply-form', plugin_dir_url( __FILE__ ) . 'my-script.js', array( 'jquery'), null, true);
        wp_localize_script('apply-form', 'settings', array('ajaxurl' => admin_url('admin-ajax.php' ),));
      }
    ...
    function myplugin_settings_date_field_callback() {
      ...
      <input type="date" class="MyDate" name="myplugin_settings_date_field" value=<?php echo esc_attr( $myplugin_date_field ); ?> >
      <?php
    }
    ...
    function jobs_expiry_date_metabox_callback($post){
      ...
      ?>
      <label for= "jobs_expiry_date"><?php ('Expiry Date'); ?></label>
      <input type="date" class="MyDate" name="jobs_expiry_date" value=<?php echo esc_attr($jobs_expiry_date); 
      ?> >
      <?php
    }
    ...
    function job_display_post( $msg ) {
      ...
      $input .= "<div class='sp_textarea'><b>Description </b>: $myplugin_textarea_field</div>";
      $input .= "<div class='sp_select'><b>Vacancies </b>: $myplugin_select_field</div>";
      $input .= "<div class='sp_title_meta'><b>Job Title </b>: $title_meta</div>";
      $input .= $date ? "<div class='sp_obs_expiry_date_meta'><b>Expiry Date </b>: $date</div>" : '';
    ...
    function job_post($content){
      ...
      $post_delete = "<p><b><i><div class = 'display_cpost'>$other_content </div>";
      ...
      <button class="delete_app" id ="delete_app" data-post_id="' . get_the_ID() . '" data-id="'.get_the_ID(). '"  class="delete-post">DELETE</button>';
       return $content . $post_delete .$button ;
    }
